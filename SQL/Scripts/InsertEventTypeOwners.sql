...
-- 1. Insert EventType Owners
BEGIN TRY
    BEGIN TRANSACTION;

    DECLARE @Now DATETIME = GETUTCDATE();
    DECLARE @SysadminID BIGINT = 1; -- Ide ĂˇltalĂˇban a rendszerfelhasznĂˇlĂłt (1) tesszĂĽk, ha nincs megadva mĂˇs

    -- TĂ¶rĂ¶ljĂĽk az esetleges duplikĂˇtumokat (biztonsĂˇgbĂłl)
    DELETE FROM [EJ].[tblEventTypeOwner] WHERE EventTypeID IN (43, 44, 45, 46) AND UserID IN (3, 197, 198);

    -- BeszĂşrĂˇsok
    INSERT INTO [EJ].[tblEventTypeOwner] (EventTypeID, UserID, CanAddOwnerFlg, ActiveFlg, LastUpdatedUserID, createdAt, updatedAt)
    VALUES 
    (43, 3, 1, 1, @SysadminID, @Now, @Now),
    (44, 3, 1, 1, @SysadminID, @Now, @Now),
    (45, 3, 1, 1, @SysadminID, @Now, @Now),
    (46, 3, 1, 1, @SysadminID, @Now, @Now),
    
    (46, 197, 1, 1, @SysadminID, @Now, @Now),
    (46, 198, 1, 1, @SysadminID, @Now, @Now);

    COMMIT TRANSACTION;
    PRINT 'Sikeresen beszĂşrva a tblEventTypeOwner adatok.';
END TRY
BEGIN CATCH
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            (13, 'SocialLogins'),
            (14, 'MaterialTypes'),
            (15, 'OwnedEventTypeIDs'); -- ĂšJ HOZZĂ ADVA
            ...
            -- ĂšJ RS: OwnedEventTypeIDs (Mikhez van jogom szervezĹ‘kĂ©nt?)
            SELECT EventTypeID FROM [EJ].[tblEventTypeOwner] WHERE UserID = @UserID AND ActiveFlg = 1;
        END
    END TRY
    BEGIN CATCH
        SET @ReturnValue = -1
